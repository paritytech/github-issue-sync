...
const main = async () => {
  ...
  const logger = new Logger({ logFormat, name: "github-issue-sync", minLogLevel: "debug" });
  ...
  switch (logFormat) {
    case "json": {
      probotLogger = getLog({ logFormat, level: "info", logLevelInString: true, logMessageKey: "msg" });
      break;
    }
  }
