...
type WebhookHandler<E extends WebhookEvents> = (ctx: Context, event: WebhookEvent<E>) => void | Promise<void>;

const setupEvent = <E extends WebhookEvents>(ctx: Context, eventName: E, handler: WebhookHandler<E>) => {
...
export const setupBotEvents = (ctx: Context) => {
  for (const issueEvent of ["issues.opened", "issues.labeled", "issues.reopened"] as const) {
    setupEvent(ctx, issueEvent, async ({ logger }, event) => {
      const { payload } = event;

      const rules = await ctx.database.wrap(async (client) => {
        const { rows }: { rows: IssueToProjectFieldRule[] } = await client.query(
          `
      ...
      const octokit = ctx.github.getInstallationOctokit(logger, payload.installation?.id);
