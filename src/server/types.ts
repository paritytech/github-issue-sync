...
import { WithDatabaseClient, WithDatabaseClientForDynamicQuery } from "./database";

export type Context = {
  bot: Probot;
  github: {
    getInstallationOctokit: (logger: Logger, installationId: number | undefined) => ExtendedOctokit;
  };
  logger: Logger;
  database: {
...
type IssueToProjectFieldRuleUpdateInput = IssueToProjectFieldRuleCreationInput & {
  github_owner: string;
  github_name: string;
};
