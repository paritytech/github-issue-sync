...
const acquirePoolClient = (pool: pg.Pool, logger: Logger) => async () => {
  let client: pg.PoolClient | undefined = undefined;

  acquireClientLoop: for (let i = 0; i < 3; i++) {
    const acquiredClient: pg.PoolClient & { hasTestQueryWorked?: true } = await pool.connect();
    ...
    if (acquiredClient.hasTestQueryWorked) {
      ...
      for (let j = 0; j < 3; j++) {
        try {
          ...
          logger.warn(`Acquired client has failed the test query on try #${j}. Retrying...`);
        }
        if (acquiredClient.hasTestQueryWorked) {
          client = acquiredClient;
          break acquireClientLoop;
          ...
          logger.info("Test query failed upon acquiring database client. Retrying...");
      ...
      try {
        /* FIXME: We have to manually make the "end" method available in the
           ...
           include it. */
        await (acquiredClient as unknown as { end: () => Promise<void> }).end();
      } finally {
...
export const withDatabaseClientForDynamicQueryCallback =
  <T>(pool: pg.Pool, logger: Logger) =>
  async (...[inputParams, fn]: Parameters<WithDatabaseClientForDynamicQuery<T>>) => {
    const queryParams = inputParams.map((change, i) => {
      return { ...change, placeholder: `$${i + 1}` };
    });

    const columns = queryParams.map(({ column }) => column);

    const paramsPlaceholdersJoined = queryParams
      .reduce((acc, { placeholder }) => `${acc}, ${placeholder}`, "")
      .slice(1)
      .trim();

    const values = queryParams.map(({ value }) => value);

    return await withDatabaseClientCallback<T, WrapForDynamicQueryContext>(pool, logger, (client) => {
      const updateStatementParamsJoined = queryParams
        .reduce((acc, { placeholder, column }) => `${acc}, ${client.escapeIdentifier(column)} = ${placeholder}`, "")
        .slice(1)
        .trim();
      const columnsJoined = columns
        .reduce((acc, v) => `${acc}, ${client.escapeIdentifier(v)}`, "")
        .slice(1)
        .trim();
      return { paramsPlaceholdersJoined, updateStatementParamsJoined, values, columnsJoined, queryParams };
    })(fn);
  };
