...
export const getOctokit = (
  ...
  if ((octokit as ExtendedOctokit)[wasOctokitExtendedByApplication] === true) {
      ...
      for (let tryCount = 1; tryCount < 4; tryCount++) {
        ...
        const waitDuration = (() => {
          ...
          if (Number(rateLimitRemainingHeader) === 0) {
            ...
            const rateLimitResetHeaderValue = response.headers[rateLimitResetHeader];
        ...
        if (waitDuration) {
          logger.warn(`Waiting for ${waitDuration}ms until requests can be made again...`);
